...
class TardisTests(GitTarget):
    ...
    @property
    def install_command(self):
        return "conda install -c conda-forge pyne=0.7 pyside2 sphinxcontrib-bibtex=1.0 sphinxcontrib-apidoc nbsphinx sphinx_bootstrap_theme && pip install sphinxcontrib-tikz dokuwiki dot2tex sphinx-jsonschema git+https://github.com/Naereen/dot2tex.git pickle5 && python setup.py install"
    
    @property
    def test_command(self):
        return "pytest tardis --no-update-deps --cov=tardis --cov-report=xml --cov-report=html"
